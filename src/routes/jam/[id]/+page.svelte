...
<main class="h-full w-full">
	<Collapsible.Root open={true} class="flex h-full w-full flex-row">
		<div class="content flex h-full w-full flex-col">
			<div class="player h-full w-full">
				<Instrument type="keyboard" />
			</div>
			<div
				class="toolbar flex w-full flex-row items-center justify-center gap-2 p-4"
			>
				<Toggle.Root
					class="btn btn-circle btn-lg btn-primary"
					bind:pressed={midiEnabled}
				>
					<MusicNoteIcon />
				</Toggle.Root>
				<Toggle.Root
			...
			{#snippet child({ props, open })}
				{#if open}
					<div
						{...props}
						transition:fly={{ x: 10 }}
						class="side h-full w-96 p-4"
					>
						<div
							class="card chat flex h-full w-full flex-col gap-2 bg-base-200 p-2"
						>
